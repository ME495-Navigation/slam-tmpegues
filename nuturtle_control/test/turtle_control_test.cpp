...
TEST_CASE("Test zero twist to wheel cmd", "[integration]")
{
    ...
    auto wheel_cmd_sub_ = node->create_subscription<nuturtlebot_msgs::msg::WheelCommands>("wheel_cmd", 10, wheel_cmd_cb_);
    ...
    (
        rclcpp::ok() &&
        ((rclcpp::Clock().now() - start_time)  < rclcpp::Duration::from_seconds(TEST_DURATION)) && !is_msg_received
    )
        {
