...
class circle : public rclcpp::Node
{
    public: circle() : Node("circle")
    {
        ...
        timer_ = this->create_wall_timer(std::chrono::milliseconds(1000 / frequency), std::bind(&circle::timer_cb_, this));
        cmd_vel_pub_ = this->create_publisher<geometry_msgs::msg::Twist>("cmd_vel", 10);
        control_service_ = this->create_service<nuturtle_control_interfaces::srv::Control>("control", std::bind(&circle::control_cb_, this, std::placeholders::_1, std::placeholders::_2));
    ...
    private:
        ...
        void control_cb_(const std::shared_ptr<nuturtle_control_interfaces::srv::Control::Request> request, [[maybe_unused]] const std::shared_ptr<nuturtle_control_interfaces::srv::Control::Response> response)
        {
        ...
        {
            stopped = !stopped;
            RCLCPP_INFO_STREAM(this->get_logger(), "Stop service, circle node stopped is now (0 stopped, 1 moving): " << !stopped);
        }
