...
public:
      ...
      : Node("nusim")
  {
    ...
    case false:
    {
      ...
      auto wc1 = turtlelib::Point2D(arena_x_length / 2, arena_y_length / 2);
      auto wc2 = turtlelib::Point2D(-arena_x_length / 2, arena_y_length / 2);
      auto wc3 = turtlelib::Point2D(-arena_x_length / 2, -arena_y_length / 2);
      auto wc4 = turtlelib::Point2D(arena_x_length / 2, -arena_y_length / 2);

      walls.push_back(std::make_pair(wc1, wc2));
      walls.push_back(std::make_pair(wc2, wc3));
      walls.push_back(std::make_pair(wc3, wc4));
      walls.push_back(std::make_pair(wc4, wc1));
...
private:
  ...
  std::vector<std::pair<turtlelib::Point2D, turtlelib::Point2D>> walls;
  ...
  void fake_laser()
  {
    ...
    for (unsigned int a = 0; a * angle_increment + laser_msg.angle_min <= laser_msg.angle_max;
         a++)
    {
      ...
      if (!angle_has_hit)
      {
        // If angle didn't hit an obstacle, check walls
        for (int w = 0; !angle_has_hit && w <= 3; w++)
        {
          auto laser_result = laser_calc.line_check(angle, slipped_dd.get_transform(), walls.at(w));
          if (laser_result.first)
          {
            angle_has_hit = true;
            laser_msg.ranges.push_back(laser_result.second);
